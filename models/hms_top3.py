...
class Net(nn.Module):
    def __init__(
        ...
        rotary_emb = LlamaRotaryEmbedding(
            encoder_dim // num_attention_heads,
        ...
        while self.cos.dim() < 4:
            self.cos = nn.Parameter(self.cos.unsqueeze(0), requires_grad=False)
        while self.sin.dim() < 4:
            self.sin = nn.Parameter(self.sin.unsqueeze(0), requires_grad=False)
        ...
        self.head1 = nn.Linear(encoder_dim, num_classes)
        self.head2 = nn.Linear(encoder_dim, 2)
        self.head3 = nn.Linear(encoder_dim, 4)

        print("n_params:", count_parameters(self))

    def forward(self, x, pad_mask=None):
        bs, n_sensors, seq_len, channels = x.shape
        mask = torch.ones(bs, seq_len, dtype=torch.long, device=x.device)
        x = x.permute(0, 1, 3, 2).contiguous()
        x = x.view(bs, 1, n_sensors * channels, seq_len)
        x = self.cnn(x)                                 # (bs, C', H', W')
        x = x.permute(0, 1, 3, 2)                       # (bs, C', W', H')
        bs, c_out, w_out, h_out = x.shape
        x = x.reshape(bs, c_out * w_out, h_out)         # (bs, F, H')
        x = x.unsqueeze(-1)                             # (bs, F, H', 1)
        x = self.global_pool(x)[:, :, 0, 0]             # (bs, F)
        x = x.view(bs, c_out, w_out).permute(0, 2, 1)   # (bs, W', F_step)
        x, _ = self.encoder(x, self.cos, self.sin, mask)# (bs, W', d_model)
        x = x.unsqueeze(1).permute(0, 3, 1, 2)          # (bs, d_model, 1, W')
        x = self.global_pool(x)[:, :, 0, 0]             # (bs, d_model)
