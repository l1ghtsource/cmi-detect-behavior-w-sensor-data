...
from modules.transformer_block import MultiHeadSelfAttention
...
class EfficientNet1dExtractor(nn.Module):
    def __init__(
        self,
        channels=cfg.imu_vars,
        hidden_dim=32,
        depth_multiplier=4,
        stem_kernel_size=3,
    ...
    def forward(self, x: torch.Tensor, pad_mask=None) -> torch.Tensor:
        # x: (bs, n_sensors, channels, seq_len)
        x = x.permute(0, 3, 1, 2) 
        x = self.encoder(x)
        bs, C, S, T = x.shape
        x = x.view(bs, C * S, T)
