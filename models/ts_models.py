...
class TS_IMUModel(nn.Module):
    def __init__(self, imu_features, num_classes, hidden_dim=128):
        ...
        self.temporal_attn = nn.TransformerEncoderLayer(hidden_dim * 2, 8, dropout=0.2, batch_first=True)
    ...
    def forward(self, imu):
        ...
        temporal_out = self.temporal_attn(imu)
...
class TS_Demo_IMUModel(nn.Module):
    def __init__(self, imu_features, demo_features=7, 
                 num_classes=18, hidden_dim=128):
        super().__init__()
        ...
        self.adaptive_fusion = nn.Sequential(
            nn.Linear(hidden_dim * 4 + hidden_dim, hidden_dim * 3), # 512 + 128 = 640
            nn.GELU(),
            nn.Dropout(0.2),
            nn.Linear(hidden_dim * 3, hidden_dim * 4) # [B, 512]
        )
        
        self.fc = nn.Sequential(
            nn.Linear(hidden_dim * 2 * 2 + hidden_dim, 512), # 256 + 128 = 384
            nn.GELU(),
            nn.Dropout(0.3),
            nn.Linear(512, 256),
            nn.GELU(),
    ...
    def forward(self, imu, demographics):
        imu = self.imu_cnn(imu.permute(0, 2, 1)).permute(0, 2, 1)
        imu, _ = self.imu_lstm(imu)
        imu = self.imu_attn(imu)
        ...
        sensor_pooled = torch.cat([
            temporal_out.mean(dim=1), 
            temporal_out.max(dim=1).values
        ], dim=1)  # [batch_size, hidden_dim * 2 * 2]
...
class TS_Demo_SimpleIMUModel(nn.Module):
    def __init__(self, imu_features, demo_features=7, 
                 num_classes=18, hidden_dim=64, dropout=0.2):
        super().__init__()
        ...
        total_features = hidden_dim * 2 + hidden_dim  # 2imu + 1demo
        self.fc = nn.Sequential(
            nn.Linear(total_features, hidden_dim * 2),
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, imu, demographics):
        ...
        sensor_features = imu_out[:, -1, :]  # [batch_size, hidden_dim * 2]
