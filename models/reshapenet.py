...
class MultiSensor_ReshapeNet_v1(nn.Module):
    def __init__(self, num_classes=cfg.main_num_classes):
        super().__init__()
        self.model = timm.create_model('efficientnet_b5', pretrained=True, in_chans=3)
        self.pool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, imu, thm, tof, pad_mask=None):
        ...
        tof = tof.permute(0, 2, 1, 3).contiguous().view(bs, 1, 120, -1)
        x = torch.cat([imu, thm, tof], dim=3)
        x = torch.cat([x, x, x], dim=1) # (bs, 3, 120, 357)
        x = self.extract_features(x)
        x = self.pool(x)
        x = x.view(bs, -1)
        x = self.dropout(x)
