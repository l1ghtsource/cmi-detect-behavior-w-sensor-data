...
def channel_wtfmix1d_batch(batch, alpha=1.0, device='cuda'):
    ...
    for key in batch.keys():
        if key not in target_keys:
            ...
            if len(original_data.shape) == 4:  # (B, sensors, seq_len, channels)
                C = original_data.size(-1)
                total_channels += C
                keep_a = np.random.choice([0, 1], size=C)  # 0=B, 1=A
                from_a_channels += keep_a.sum()
                mixed_data = original_data.clone()

                for c in range(C):
                    if keep_a[c] == 0:
                        mixed_data[..., c] = original_data[index][..., c]
    ...
    lam = from_a_channels / total_channels
