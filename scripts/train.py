...
def get_oof_predictions(model, val_loader, output_index=0):
    model.eval()
    all_preds = []
    with torch.no_grad():
        for batch in val_loader:
            ...
            if output_index == 0:
                ...
                selected_output = ext1_out1
            elif output_index == 2:
                selected_output = ext2_out1
            elif output_index == 3:
                selected_output = ext3_out1
            elif output_index == 4:
...
def run_training_with_stratified_group_kfold():
    ...
    oof_preds_ext1_top1 = np.zeros((len(train_seq), cfg.main_num_classes))
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(train_seq, targets, groups)):
        ...
        preds_top1 = get_oof_predictions(model, val_loader, output_index=0)
        oof_preds_top1[val_idx] = preds_top1
        
        preds_ext1_top1 = get_oof_predictions(model, val_loader, output_index=1)
        oof_preds_ext1_top1[val_idx] = preds_ext1_top1
        
        preds_ext2_top1 = get_oof_predictions(model, val_loader, output_index=2)
        oof_preds_ext2_top1[val_idx] = preds_ext2_top1
        
        preds_ext3_top1 = get_oof_predictions(model, val_loader, output_index=3)
        oof_preds_ext3_top1[val_idx] = preds_ext3_top1
        ...
        preds_top3_avg = get_oof_predictions(model, val_loader, output_index=0)
        oof_preds_top3_avg[val_idx] = preds_top3_avg
        
        preds_ext1_top3_avg = get_oof_predictions(model, val_loader, output_index=1)
        oof_preds_ext1_top3_avg[val_idx] = preds_ext1_top3_avg
        
        preds_ext2_top3_avg = get_oof_predictions(model, val_loader, output_index=2)
        oof_preds_ext2_top3_avg[val_idx] = preds_ext2_top3_avg
        
        preds_ext3_top3_avg = get_oof_predictions(model, val_loader, output_index=3)
        oof_preds_ext3_top3_avg[val_idx] = preds_ext3_top3_avg
        ...
        preds_top5_avg = get_oof_predictions(model, val_loader, output_index=0)
        oof_preds_top5_avg[val_idx] = preds_top5_avg
        
        preds_ext1_top5_avg = get_oof_predictions(model, val_loader, output_index=1)
        oof_preds_ext1_top5_avg[val_idx] = preds_ext1_top5_avg
        
        preds_ext2_top5_avg = get_oof_predictions(model, val_loader, output_index=2)
        oof_preds_ext2_top5_avg[val_idx] = preds_ext2_top5_avg
        
        preds_ext3_top5_avg = get_oof_predictions(model, val_loader, output_index=3)
        oof_preds_ext3_top5_avg[val_idx] = preds_ext3_top5_avg
    ...
    for i, ext_name in enumerate(extractor_names):
        ext_pred_labels_top1 = np.argmax(extractor_oof_preds_top1[i], axis=1)
        ext_m_top1, ext_bm_top1, ext_mm_top1 = comp_metric(oof_targets, ext_pred_labels_top1)
        
        ext_pred_labels_top3_avg = np.argmax(extractor_oof_preds_top3_avg[i], axis=1)
        ext_m_top3_avg, ext_bm_top3_avg, ext_mm_top3_avg = comp_metric(oof_targets, ext_pred_labels_top3_avg)
        
        ext_pred_labels_top5_avg = np.argmax(extractor_oof_preds_top5_avg[i], axis=1)
        ext_m_top5_avg, ext_bm_top5_avg, ext_mm_top5_avg = comp_metric(oof_targets, ext_pred_labels_top5_avg)
        
        extractor_metrics[ext_name] = {
            'top1_model': {
                'oof_avg_f1': ext_m_top1,
                'oof_binary_f1': ext_bm_top1,
                'oof_macro_f1': ext_mm_top1
            },
            'top3_avg_model': {
                'oof_avg_f1': ext_m_top3_avg,
                'oof_binary_f1': ext_bm_top3_avg,
                'oof_macro_f1': ext_mm_top3_avg
            },
            'top5_avg_model': {
                'oof_avg_f1': ext_m_top5_avg,
                'oof_binary_f1': ext_bm_top5_avg,
                'oof_macro_f1': ext_mm_top5_avg
            }
        }
        
        print(f'{ext_name} - use top1 models: {ext_m_top1=}, {ext_bm_top1=}, {ext_mm_top1=}')
        print(f'{ext_name} - use top3 avg models: {ext_m_top3_avg=}, {ext_bm_top3_avg=}, {ext_mm_top3_avg=}')
        print(f'{ext_name} - use top5 avg models: {ext_m_top5_avg=}, {ext_bm_top5_avg=}, {ext_mm_top5_avg=}')
    
    print(f'main - use top1 models: {oof_m=}, {oof_bm=}, {oof_mm=}')
    print(f'main - use top3 avg models: {oof_m_top3_avg=}, {oof_bm_top3_avg=}, {oof_mm_top3_avg=}')
    print(f'main - use top5 avg models: {oof_m_top5_avg=}, {oof_bm_top5_avg=}, {oof_mm_top5_avg=}')
    ...
    for i, ext_name in enumerate(extractor_names):
        np.save(os.path.join(cfg.oof_dir, f'{prefix}oof_preds_{ext_name}_top1.npy'), extractor_oof_preds_top1[i])
        np.save(os.path.join(cfg.oof_dir, f'{prefix}oof_pred_labels_{ext_name}_top1.npy'), np.argmax(extractor_oof_preds_top1[i], axis=1))
        
        np.save(os.path.join(cfg.oof_dir, f'{prefix}oof_preds_{ext_name}_top3_avg.npy'), extractor_oof_preds_top3_avg[i])
        np.save(os.path.join(cfg.oof_dir, f'{prefix}oof_pred_labels_{ext_name}_top3_avg.npy'), np.argmax(extractor_oof_preds_top3_avg[i], axis=1))
        
        np.save(os.path.join(cfg.oof_dir, f'{prefix}oof_preds_{ext_name}_top5_avg.npy'), extractor_oof_preds_top5_avg[i])
        np.save(os.path.join(cfg.oof_dir, f'{prefix}oof_pred_labels_{ext_name}_top5_avg.npy'), np.argmax(extractor_oof_preds_top5_avg[i], axis=1))
    ...
    oof_info = {
        ...
        'extractor_outputs': extractor_metrics,
        'cv_statistics': {
