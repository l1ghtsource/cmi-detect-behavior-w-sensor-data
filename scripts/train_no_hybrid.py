...
def train_epoch(train_loader, model, optimizer, main_criterion, seq_type_criterion, orientation_criterion, device, scheduler, ema=None, current_step=0, num_warmup_steps=0, fold=None):
    ...
    for batch in loop:
        ...
        if cfg.use_mixup and curr_proba > cfg.mixup_proba and not is_warmup_phase:
            ...
            main_loss, seq_type_loss, orientation_loss = mixup_criterion(
...
def run_training_with_stratified_group_kfold():
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(train_seq, targets, groups)):
        ...
        for epoch in range(cfg.n_epochs):
            ...
            val_loss, avg_m_val, bm_val, mm_val, all_targets, all_preds = valid_epoch(val_loader, model, main_criterion, seq_type_criterion, orientation_criterion, device, ema)
        ...
        del model, optimizer, scheduler, main_criterion, seq_type_criterion, orientation_criterion
        if ema is not None:
            del ema
